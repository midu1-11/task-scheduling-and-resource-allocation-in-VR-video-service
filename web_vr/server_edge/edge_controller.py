...
class Controller:
    ...
    # 返回任务调度结果
    def get_decision_list(self, resolution_list):
        b_d = []
        data_size_list = []
        for i in range(6):
            data_size_list.append(((int)(resolution_list[i].split("x")[1]) - 240) / 840.0)
        for i in range(6):
            b_d.append([0.086 * data_size_list[i] + 0.014, 1.36 * data_size_list[i] + 0.4])
