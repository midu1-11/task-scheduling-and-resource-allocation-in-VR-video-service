...
public class MyDatacenterBroker extends SimEntity {
    ...
    protected void processCloudletReturn(SimEvent ev) {
        MyCloudlet mycloudlet = (MyCloudlet) ev.getData();

        //设置端设备收到任务的时间
        mycloudlet.setSolveTime(CloudSim.clock());

        //计算整个任务的开销
        double cost = 0.5 * (mycloudlet.getSolveTime() - mycloudlet.getGenerateTime()) + 0.5 *
                (mycloudlet.isExcutedByCloud() ? Double.parseDouble(SysConfig.getInfo
                        ("cloudCostPerCpuCircle")) : Double.parseDouble(SysConfig.getInfo
                        ("mecCostPerCpuCircle"))) * mycloudlet.getCloudletLength();
        mycloudlet.setCost(cost);

        getCloudletReceivedList().add(mycloudlet);
        Log.printConcatLine(CloudSim.clock(), ": ", getName(), ": Cloudlet ", mycloudlet.getCloudletId(),
                " received");
        cloudletsSubmitted--;
...
//        }
     ...
     * 这个方法的作用是依次处理cloudletList里面的任务，将其发给对应的vm，最后清空cloudletList。
     *
     * @pre $none
     * @post $none
     * @see #submitCloudletList(java.util.List)
    ...
    public void submitCloudlets() {
        ...
        for (Cloudlet cloudlet : getCloudletList()) {
            ...
            //这里delay是考虑了端设备的发送延迟
            double delay = cloudlet.getCloudletFileSize() * 1024 * 8 / (double) getBw();
