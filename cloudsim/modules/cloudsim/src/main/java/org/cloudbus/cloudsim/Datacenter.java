...
public class Datacenter extends SimEntity {
	...
	protected void updateCloudletProcessing() {
		...
		// 这里我修改了一下，原来这里的判断是
		// if (CloudSim.clock() < 0.111 || CloudSim.clock() >=
		// getLastProcessTime() + CloudSim.getMinTimeBetweenEvents())
		// 后一项是保证两次处理之间间隔最小时间，但是貌似并不会触发，这里改成true方便在EdgeServer的
		// updateCloudletProcessing方法中连续调用此函数
		if (CloudSim.clock() < 0.111 || true) {
	...
	protected void updateCloudletProcessing1() {
		// if some time passed since last processing
		// R: for term is to allow loop at simulation start. Otherwise, one initial
		// simulation step is skipped and schedulers are not properly initialized
